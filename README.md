...
**jvpm** は jv 言語のパッケージマネージャーですが、単体で利用する場合は `mvn` コマンドの完全なラッパーとして動作し、既存の Java/Maven プロジェクト管理にも利用できます。`pom.xml` のあるディレクトリで実行すると自動的にラッパーモードで動作し、PubGrub による高速な依存解決と `jv.lock` によるバージョン固定を提供します。
