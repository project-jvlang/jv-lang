...
Rowan パイプラインから Salsa パイプラインへ移行するための手順を示す。
...
2. **パイプライン切替**: フロントエンド呼び出し箇所で `RowanPipeline` を `SalsaPipeline` に差し替え、必要に応じて `ParseOptions` で CST/Trivia を有効化する。
3. **診断正規化**: テストで `support::normalize` を利用し、Rowan との差分を検証する。
...
- スパン差分が許容範囲外の場合は、`spans_within_tolerance` の閾値を見直し、Rowan 側のスパン計算と同期する。
