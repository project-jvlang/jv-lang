...
> **Maven ラッパーモード**: 既存の Maven プロジェクトで jvpm を使用する場合は、[jvpm Maven ラッパーガイド](jvpm-maven-wrapper.md) を参照してください。`jvpm init` で新規 Maven プロジェクトを作成したり、`jvpm install` で依存解決と Maven ビルドを一括実行できます。
