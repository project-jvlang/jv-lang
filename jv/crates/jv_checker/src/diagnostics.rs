...
const DIAGNOSTICS: &[DiagnosticDescriptor] = &[
    ...
    DiagnosticDescriptor {
        code: "JV_REGEX_E201",
        title: "正規表現リテラルが未終端です / Regex literal is unterminated",
        help: "末尾に `/` を追加してリテラルを閉じてください。参照: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html / Add the trailing `/` to close the literal. Reference: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_E202",
        title: "正規表現リテラルの括弧が不均衡です / Regex literal contains unbalanced grouping",
        help: "括弧・ブラケット・ブレースの対応を確認し、対応する記号を追加または削除してください。参照: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html / Review bracket pairs and adjust the symbols so they match. Reference: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_E203",
        title: "サポートされない正規表現エスケープです / Unsupported regex escape sequence",
        help: "Java互換のエスケープ（例: \\n, \\t, \\\\）へ置き換えてください。参照: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html / Replace the sequence with a Java-compatible escape such as \\n, \\t, or \\\\. Reference: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_W301",
    ...
    DiagnosticDescriptor {
        ...
        help: "パターンを簡素化するか部分的に分割して検証コストを抑えてください。参照: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html / Simplify the pattern or split it into smaller parts to reduce validation time. Reference: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/regex/Pattern.html",
        severity: DiagnosticSeverity::Information,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_W001",
    ...
    DiagnosticDescriptor {
        code: "JV_REGEX_I002",
        title: "正規表現リテラルのモードがあいまいです / Regex literal has ambiguous mode",
        help: "正規表現リテラルが置換なのか判定なのか不明です。置換なら `//` を追加し、判定なら `[match]` を明示してください。/ The regex literal cannot tell whether you intend a replacement or a predicate. Add `//` for replacements or `[match]` for predicate mode.",
        severity: DiagnosticSeverity::Information,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_I003",
        title: "正規表現リテラル結果を大規模入力で配列化しています / Regex literal result materialises large input",
        help: "正規表現リテラルの結果を `as List` や `split` で一括配列化すると大容量入力でメモリを圧迫します。`Stream` のまま処理するか `limit` を利用してください。/ Materialising regex literal results with `as List` or `split` can exhaust memory on large inputs. Prefer streaming or constrain the result with `limit`.",
        severity: DiagnosticSeverity::Information,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_I004",
        title: "小規模な正規表現リテラル結果はリスト化できます / Materialise small regex literal results",
        help: "少量の正規表現リテラル結果であれば `as List` でリスト化すると再利用が容易になります。/ When a regex literal produces only a few matches, materialising with `as List` can simplify reuse.",
        severity: DiagnosticSeverity::Information,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_I005",
        title: "正規表現リテラルの `Stream<MatchResult>` は再利用できません / Regex literal Stream<MatchResult> cannot be reused",
        help: "正規表現リテラルが返す `Stream<MatchResult>` は一度きりです。複数回利用する場合は `as List` で結果を素材化してください。/ The `Stream<MatchResult>` produced by a regex literal can only be consumed once. Materialise it with `as List` if you need to reuse the matches.",
        severity: DiagnosticSeverity::Information,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_E101",
        title: "正規表現リテラルで未知のフラグが指定されました / Unknown flag supplied to regex literal",
        help: "正規表現リテラルで利用可能なフラグは `i`, `m`, `s`, `u`, `d`, `x`, `l`, `c` です。/ Regex literals accept only the flags `i`, `m`, `s`, `u`, `d`, `x`, `l`, `c`.",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_E102",
        title: "正規表現リテラルのラムダ戻り値が String ではありません / Regex literal lambda must return String",
        help: "正規表現リテラルのラムダ置換は `String` を返す必要があります。`toString()` で文字列化するか `String.valueOf(...)` を利用してください。/ Regex literal replacement lambdas must return `String`. Convert the value with `toString()` or `String.valueOf(...)`.",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_E103",
        title: "正規表現リテラルで排他的なフラグが同時指定されています / Mutually exclusive regex literal flags used together",
        help: "競合するフラグを同時に使用できません。どちらか片方のみを残してください。/ Mutually exclusive regex literal flags cannot be combined. Remove one of them.",
        severity: DiagnosticSeverity::Error,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_W003",
        title: "正規表現リテラル置換ラムダが外側の変数を変更しています / Regex literal replacement lambda mutates outer state",
        help: "副作用を避けるため、正規表現リテラルの置換ラムダ内では独立した変数を利用するか同期機構を導入してください。/ To avoid side effects, let regex literal replacement lambdas operate on independent variables or guard mutations with synchronization.",
        severity: DiagnosticSeverity::Warning,
    },
    DiagnosticDescriptor {
        code: "JV_REGEX_S001",
        title: "正規表現リテラルの結果を外部 API へ直接渡しています / Regex literal output is sent directly to an external API",
        help: "外部システムへ送信する前に正規表現リテラルの結果をサニタイズや検証で安全確認してください。/ Sanitise or validate regex literal output before sending it to external systems.",
        severity: DiagnosticSeverity::Warning,
    },
    DiagnosticDescriptor {
        code: "JV3199",
