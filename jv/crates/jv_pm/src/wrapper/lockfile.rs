...
impl WrapperLockfileWriter {
    pub fn write_lockfile(
        ...
        let lockfile = metrics::measure("wrapper-lockfilewriter-generation", || {
            LockfileService::generate_from_resolved(resolved).map_err(|error| {
                WrapperError::OperationFailed(format!("jv.lock の生成に失敗しました: {error}"))
            })
        })?;

        let lockfile_path = project_root.as_ref().join("jv.lock");
        metrics::measure("wrapper-lockfilewriter-write", || {
