...
use super::support::{
    identifier, keyword, span_from_token, token_comma, token_if, token_left_paren,
    token_right_paren,
};
    ...
    recursive(move |pattern| {
        ...
        let is_pattern =
            keyword("is")
                ...
                .map(|type_name| Pattern::Constructor {
                    name: type_name,
                    patterns: Vec::new(),
                    span: Span::default(),
                });

        let identifier_or_constructor = identifier()
            .then(
                token_left_paren()
                    .ignore_then(pattern.clone().separated_by(token_comma()).allow_trailing())
                    .then_ignore(token_right_paren())
                    .or_not(),
            )
            .map(|(name, args)| match args {
                Some(arguments) => Pattern::Constructor {
                    name,
                    patterns: arguments,
                    span: Span::default(),
                },
                None => Pattern::Identifier(name, Span::default()),
            });
