...
mod suite {
    ...
    fn rowan_pipeline_produces_generic_signature() {
        ...
        let frontend = Parser::parse(source).expect("parse succeeds");
...
"#;
    ...
    fn transform_repository_fixtures_lowering_smoke() {
        ...
        for path in files {
            ...
            let frontend = match Parser::parse(&source) {
    ...
    fn transform_quick_tour_lowers_successfully() {
        ...
        let frontend = Parser::parse(source).expect("quick-tour example should parse");
        let program = frontend.into_program();
        let mut context = TransformContext::new();
        transform_program_with_context(program, &mut context)
    ...
    fn transform_examples_without_failure() {
        ...
        for path in files {
            ...
            let frontend = Parser::parse(&source)
                .unwrap_or_else(|err| panic!("Parser failed for {}: {:?}", display, err));
            let program = frontend.into_program();
            let mut context = TransformContext::new();
            if let Err(err) = transform_program_with_context(program, &mut context) {
