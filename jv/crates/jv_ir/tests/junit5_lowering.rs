...
fn lowers_parameterized_test_with_dataset_provider() {
    ...
    let (test_method, provider_method) = if let IrStatement::MethodDeclaration {
        modifiers,
        ..
    } = methods[0]
    {
    ...
    } = test_method
    {
        ...
        assert!(
            matches!(assertion_patterns.as_slice(), [AssertionPattern::Equals { .. }]),
            "dataset-driven test should still classify assertions"
        );
    } else {
